...
from src.schemas.pdf_parser.models import PdfContent, ArxivMetadata, ParsedPaper
...
class MetadataFetcher:
    ...
    async def fetch_and_process_papers(
        ...
        try:
            ...
            papers : List[ArxivPaper] = await self.arxiv_client.fetch_papers(
    ...
    async def _process_pdfs_batch(self, papers: List[ArxivPaper]) -> Dict[str, Any]:
        ...
        for paper, result in zip(papers, pipeline_results):
            if isinstance(result, Exception):
                ...
                download_success, parsed_paper = result if isinstance(result, tuple) else (False, None)
    ...
    def _serialize_parsed_content(self, parsed_paper: ParsedPaper) -> Dict[str, Any]:
        ...
        try:
            ...
            if pdf_content is None:
                return {"pdf_processed": False, "parser_metadata": {"note": "PDF processing not available or failed"}}
            # Serialize sections
