...
class OpenSearchClient:
    ...
    def __init__(self, host:str='http://localhost:9200', 
        ...
        logger.info(f"OpenSearch client initialized with host {self.host}")

    def setup_indices(self, force: bool = False) -> Dict[str, bool]:
        """
        Setup the hybrid search index and RRF pipeline.

        :param force: If True, delete existing index and pipeline before creating
        :type force: bool
        :returns: Dictionary with success/failure status for index and pipeline
        :rtype: Dict[str, bool]
        """
        results = {}
    ...
    def index_paper(self, paper_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if "arxiv_id" not in paper_data:
                logger.error(f"Arxiv ID is required for indexing")
                return False
            # add timestamp fields if not present already
