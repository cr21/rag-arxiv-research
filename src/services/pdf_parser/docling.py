...
class DoclingParser:
    ...
    def __init__(self, max_pages: int = 20, max_file_size_mb: int = 20, do_ocr: bool = False, do_table_structure: bool = True):
        ...
        try:
            ...
            raise ImportError(
                "DoclingParser requires the optional `docling` extra (not installable on macOS Intel via PyPI). "
                "Use: uv sync --extra docling on Linux/ARM, or run the app in Docker (see Dockerfile)."
            ) from e
