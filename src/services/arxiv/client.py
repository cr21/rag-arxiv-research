...
class ArxivClient:
    ...
    def pdf_cache_dir(self) -> Path:
        ...
        cache_dir = Path(self.settings.pdf_cache_dir)
    ...
    def rate_limit_delay(self) -> float:
        return self.settings.rate_limit_delay
    ...
    def search_category(self) -> str:
        return self.settings.search_category
    ...
    async def fetch_papers(self,
        ...
        # Add Data Filtering If needed
        if from_date or to_date:
            # Convert dates to arXiv format (YYYYMMDDHHMM) - use 0000 for start of day, 2359 for end
            data_from =f"{from_date}0000" if from_date else "*"
            data_to =f"{to_date}2359" if to_date else "*"
            search_query += f" AND submittedDate:[{data_from}+TO+{data_to}]"
        ...
        try:
            ...
            if self._last_request_time is not None:
                ...
                if time_since_last < self.rate_limit_delay:
                    ...
                    logger.info(f"Rate limit hit. Waiting {delay:.2f} seconds before next request.")
            ...
            logger.info(f"Successfully fetched {len(papers)} {self.search_category} papers from arXiv")
            return papers
